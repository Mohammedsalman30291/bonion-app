...
const OrdersList = () => {
  const [orders, setOrders] = useState([
    ...
    },
    {
      ...
      itemDiscount: 5,
      itemDescription:
        "Delicious white chicken biriyani in dindugal style. Fills your tongue for the day.",
      itemQty: 2,
      resId: "r1",
    ...
    },
    {
      ...
      itemDiscount: 6,
      itemDescription:
        "Fully cooked grill with evenly spread Arabian style masala.",
      itemQty: 1,
      resId: "r2",
      resName: "Buhari",
      resAdd: "mount road",
  ...
  return (
    <div className="orders">
      ...
      {orders.map((order) => (
        ...
        >
          ...
          <div className="orderDetails">
            <h3>
              {order.itemName} - {order.resName}
            </h3>
            {/* <p>{order.oDate}</p> */}
            <p>
              <em>{order.itemDescription}</em>
            </p>
            <code>
              <strike>{order.itemPrice}</strike>
              &nbsp;
              {order.itemPrice - (order.itemDiscount / 100) * order.itemPrice}/-
            </code>
            <p className="orderAddress">
              Delivered to: {order.uAdd} - {order.uPin},&nbsp; Ph:{" "}
              {order.uMobile}
            </p>
          </div>
        </div>
      ...
      {selectedOrder && (
        <OrderDetails order1={selectedOrder} close={closeOrderDetails} />
      )}
    </div>
  );
};
